...
latest_frame_cv = threading.Condition()
latest_frame = None
...
@app.route('/api/reactor/prompt', methods=['POST'])
...
def reactor_health():
    return jsonify(reactor_service.get_health())

@app.route('/api/reactor/reference-image', methods=['POST'])
def reactor_reference_image():
    ...
    save_path = os.path.join(thisPath, 'reactor_reference_' + filename)
    reactor_service.set_reference_image(file.read(), filename, save_path)
